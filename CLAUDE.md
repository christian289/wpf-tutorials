...
- **MVVM이란, ViewModel class에 System.Windows로 시작하는 클래스들이 참조되지 않는 것을 의미**
- **⚠️ 절대로 FCL 객체가 참조되지 않도록 한다**
...
2. **ViewModel에 System.Windows 네임스페이스 참조** - MVVM 위반
3. **불필요한 IReadOnlyCollection<T>, HashSet<T> 사용** - 성능 향상이 명확하지 않은 경우
4. **요청하지 않은 기능까지 미리 구현** - 요청한 것만 구현, 나머지는 제안
